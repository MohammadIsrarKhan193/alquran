...
fetch("https:                              
  ...
  .then(data => {
    ...
    data.data.forEach(surah => {
      const btn = document.createElement("//api.alquran.cloud/v1/surah")
  .then(res => res.json())
  .then(data => {
    loading.remove();
    data.data.forEach(surah => {
...
function loadSurah(number, name) {
  ...
  ayahsDiv.innerHTML = "// LOAD SURAH (ARABIC + ENGLISH)
function loadSurah(number, name) {
  surahTitle.textContent = `Surah ${name}`;
  ...
  .then(([arabic, english]) => {
    ayahsDiv.innerHTML = "//api.alquran.cloud/v1/surah/${number}`).then(r => r.json()),
