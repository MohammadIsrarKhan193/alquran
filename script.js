...
async function init() {
    ...
    try {
        ...
        allSurahs = data.data; // This will always be 114
...
function displaySurahs(list) {
    ...
    list.forEach(s => {
        ...
        // Shows both English and Arabic Name
        btn.innerHTML = `<div><b>${s.number}. ${s.englishName}</b><br><small>${s.englishNameTranslation}</small></div> <span class="ar-name">${s.name}</span>`;
        btn.onclick = () => loadSurah(s.number, s.englishName);
        surahList.appendChild(btn);
    });
}

// Fixed Search: Trims spaces to ensure it finds all Surahs
...
async function loadSurah(num, name) {
    ...
    data.data[0].ayahs.forEach((ayah, i) => {
        ...
        card.innerHTML = `
            <div class="ar-line">
                <span class="arabic-text">${arText}</span>
    ...
    fetch('https://ipapi.co/json/').then(r => r.json()).then(loc => {
        ...
        .then(r => r.json()).then(d => {
            ...
            document.getElementById("times-list").innerHTML = `
                <p><b>Fajr:</b> ${t.Fajr} | <b>Dhuhr:</b> ${t.Dhuhr} | <b>Maghrib:</b> ${t.Maghrib}</p>
            `;
        });
    }).catch(() => document.getElementById("times-list").innerText = "Prayer times unavailable.");
}
